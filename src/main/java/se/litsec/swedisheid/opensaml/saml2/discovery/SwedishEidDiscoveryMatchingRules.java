...
public class SwedishEidDiscoveryMatchingRules {
  ...
  public static boolean isServiceEntityMatch(final Collection<String> consumingService, final Collection<String> providingService) {
    Collection<String> csServiceEntityCategories = consumingService.stream()
      .filter(c -> EntityCategoryType.isType(c, EntityCategoryType.SERVICE_ENTITY))
      .collect(Collectors.toList());
    if (!csServiceEntityCategories.isEmpty()) {
      String matchingServiceEntityCategory = csServiceEntityCategories.stream()
  ...
  public static boolean isServiceContractMatch(final Collection<String> consumingService, final Collection<String> providingService) {
    Collection<String> psServiceContractCategories = providingService.stream()
      .filter(c -> EntityCategoryType.isType(c, EntityCategoryType.SERVICE_CONTRACT))
      .collect(Collectors.toList());
    if (!psServiceContractCategories.isEmpty()) {
      String matchingServiceContractCategory = psServiceContractCategories.stream()
  ...
  public static boolean isServicePropertyMatch(final Collection<String> consumingService, final Collection<String> providingService) {
    Collection<String> csServicePropertyCategories = consumingService.stream()
