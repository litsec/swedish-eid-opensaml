...
public class CompositeMetadataProvider extends AbstractMetadataProvider {
  ...
  /**
   * Collects all metadata from all underlying providers and creates an {@code EntitiesDescriptor} element. Any duplicate entity ID:s will be removed.
   */
  @Override
  ...
  private synchronized void collectMetadata() {
    ...
    EntitiesDescriptor metadata = SAMLUtils.createSamlObject(EntitiesDescriptor.class);
    metadata.setName(this.getID());
    metadata.setID(SAMLUtils.generateIDAttribute("metadata_", 32));
    ...
    for (MetadataProvider provider : this.metadataProviders) {
      ...
      while (it.hasNext()) {
        ...
        if (entityIds.contains(ed.getEntityID())) {
          log.warn("EntityDescriptor for '{}' already exists in metadata. Entry read from provider '{}' will be ignored.", ed.getEntityID(), provider.getID());
          continue;
        }        
        try {
